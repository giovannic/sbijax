...
def main():
    ...
    # Load selective inference config (always present now)
    selective_config_file = out_dir / "selective_inference_config.json"
    
    # Check required files including selective config
    all_required_files = required_files + ["selective_inference_config.json"]
    ...
    # Load selective inference configuration (always present)
    ...
    # Reconstruct theta dictionaries using selective approach
    sample_params = selective_config['sample_params']
    fixed_params_list = selective_config['fixed_params']
